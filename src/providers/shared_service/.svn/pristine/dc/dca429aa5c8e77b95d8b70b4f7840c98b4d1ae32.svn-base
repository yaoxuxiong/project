...
const ULTRACREATION_AD_FLAG_1 = 0xFFBC;     // old deprecated
const ULTRACREATION_AD_FLAG_2 = 0xFFBD;
    ...
    private UpdateBattery(BatteryLevel: number): void
    {
        ...
        if (this.Status.BatteryLevel <= MinBat)
            ...
            this.Status.BatteryPercent =
                Math.round(100 * (this.Status.BatteryLevel - MinBat) / (MaxBat - MinBat));
...
export abstract class TConnectablePeripheral extends TPeripheral
{
    ...
    {
        return this.Shell.Connect();
    }

    Disconnect(): Promise<void>
    {
        return this.Shell.Disconnect();
    }
}
...
export class PeripheralFactory
{
    static Register(PeripheralClass: typeof TPeripheral)
    {
        ...
        console.log('PeripheralFactory: ' + PeripheralClass.ProductName + ' registered');
    }
    ...
    static GetFromBLE(AdName: string, data: Uint8Array, ScanId: string): TPeripheral | undefined
    {
        ...
        if (TypeInfo.Assigned(data) && data.byteLength >= 7)
        {
            ...
            if (ad === ULTRACREATION_AD_FLAG_2)
            {
                ...
                // device'id
                id = (HexConv.IntToHex(data[9], 2) + ':' + HexConv.IntToHex(data[8], 2) + ':' + HexConv.IntToHex(data[7], 2)
                    + ':' + HexConv.IntToHex(data[6], 2) + ':' + HexConv.IntToHex(data[5], 2) + ':' + HexConv.IntToHex(data[4], 2)).toUpperCase();
            ...
            else if (ad === ULTRACREATION_AD_FLAG_1)
            {
                // firmware version
                ver = data[2] * 256 + data[3];
        ...
        if (! TypeInfo.Assigned(Peripheral))
        {
            ...
            {
                const PeripheralClass = Iter.value[1] as (typeof TPeripheral);
                if (0 !== ad && ULTRACREATION_AD_FLAG_1 !== ad && ULTRACREATION_AD_FLAG_2 !== ad)
                    continue;

                if ((PeripheralClass.AdName.indexOf(AdName) !== -1) ||
                    (PeripheralClass.AdName.indexOf(NameWithCase) !== -1) ||
