...
import {TGatt, TGapConnection, TCharacteristicProp, TCharacteristicStream} from './BLE';
...
export class TShell extends TAbstractShell
{
    ...
    Connect(): Promise<void> /**@override TAbstractShel */
    {
        ...
        this.Connecting = TGatt.Connect(this._DeviceId, this.ConnectionTimeout)
            .then((Conn) =>
            {
                ...
                this.Prop = Conn.CharacteristicsInfo.get(CHARACTERISTRIC_UUID_SHELL).properties;

                Conn.OnTimeout.subscribe(next => this.OnConnectionTimeout());
    ...
    Prop: Array<TCharacteristicProp>;
