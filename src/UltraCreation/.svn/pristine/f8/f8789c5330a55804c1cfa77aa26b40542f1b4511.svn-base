...
/* IStorage */
        ...
        for (const Rule of Rules)
        {
            try
            {
                ...
                for (const str of Rule.KeyProps)
                {
                    ...
                    // KeyProps in updates
                    if (! Rule.NoUpdateKeyProps && Updates.indexOf(str) === -1)
                    {
                        if (! this.ValueEqueal(NewValue, OldValue))
                        {
                            Updates.push(str);
                ...
                for (const str of Rule.Props)
                {
                    ...
                    if ((OldObj === Instance || ! this.ValueEqueal(NewValue, OldValue)) &&
    ...
    private ValueEqueal(L: any, R: any): boolean
    {
