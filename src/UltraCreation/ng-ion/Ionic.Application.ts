...
import {TCordovaApplication} from '../Native/Cordova.Application';

@Injectable()
export class TIonicApplication extends TCordovaApplication
{
    constructor(public Injector: Injector)
    {
        ...
        /** todo: cordova 'backbutton' not working */
        const p = Injector.get(Platform);
        p.registerBackButtonAction(() =>
        {
    ...
    */
    ...
    async ShowToast(opt: ToastOptions & {prefix_lang?: string}): Promise<Toast>
    {
        if (TypeInfo.Assigned(this.Toast))
            await this.Toast.dismiss();

        if (TypeInfo.Assigned(opt) && TypeInfo.Assigned(opt.message))
        {
            let lang_id: string;
            if (TypeInfo.Assigned(opt.prefix_lang))
                lang_id = opt.prefix_lang + opt.message;
            else
                lang_id = opt.message;
    ...
    async ShowError(err: any, opt: ToastOptions & {prefix_lang?: string}): Promise<void>
    {
    ...
    {
        return TypeInfo.Assigned(this.Loading);
    }
    ...
    async ShowLoading(opt?: LoadingOptions): Promise<Loading>
    {
